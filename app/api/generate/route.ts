...
export async function POST(request: Request) {
  ...
  const model = normalizeModelId(
    String(formData.get("model") || DEFAULT_IMAGE_MODEL_ID),
  );
  const requestIp = getRequestIp(request);
  const signingSecret = getSigningSecret();
  const sessionKey = getLimiterKey("session", session.id, signingSecret);
  const ipKey = getLimiterKey("ip", requestIp, signingSecret);
  ...
  try {
    ...
    const ipRateLimit = await checkGenerationRateLimit(null, ipKey, ipLimitOptions);
    rateLimit = ipRateLimit.ok
      ? await checkGenerationRateLimit(null, sessionKey, sessionLimitOptions)
      : ipRateLimit;
    generationGuard = rateLimit.ok
      ? await acquireGenerationGuard({
          ipKey,
